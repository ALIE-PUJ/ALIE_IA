...
print("Pinecone API key set: ", pinecone_api_key)
...
def create_vector_store(docs, embedding_model):
    """
    Create a Pinecone vector database using the specified embedding model, assigning IDs to documents.
    """
    embedding_function = SentenceTransformerEmbeddings(model_name=embedding_model)
    
    import time

    index_name = "syllabus-index"  # change if desired. Take an index from https://app.pinecone.io/
    ...
    embeddings = FakeEmbeddings(size=1024) # Change size depending on pinecone index config

    vector_store = PineconeVectorStore(index=index, embedding=embeddings)
    # Pinecone.from_documents(docs, embedding_function, ids=ids)
...
specific_question5 = "Cual es el codigo de curso de Estructuras de datos"
specific_question6 = "Donde puedo encontrar el plan de estudios de mi carrera?"
specific_question7 = "What is the box-office of Jurassic park?"
...
results = vector_store.similarity_search(
    "Cual es el codigo de curso de Estructuras de datos",
    k=2
)
